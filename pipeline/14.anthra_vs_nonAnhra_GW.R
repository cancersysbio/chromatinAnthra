...
for(i in 1:nrow(eset_1_COMBAT2)){
  ...
  try({
    ...
    tmpCtrs = as.data.frame(rbind(contrast(cxmodel2,list(exp_c=min_lim,anthra=T),list(exp_c=min_lim,anthra=F)),
                                  #contrast(cxmodel2,list(exp_c=min_lim,anthra=F)),
                                  contrast(cxmodel2,list(exp_c=max_lim,anthra=T),list(exp_c=max_lim,anthra=F))
                                  #contrast(cxmodel2,list(exp_c=max_lim,anthra=F)),
    ),stringAsFactors=F)
...
save( mulvarrfs.res,file="../data/metacohort_assoc_results_age_cohort_er_pr_her2_tsaeg_ln_anthraVSnoAnthra_GW.RData")
save(contrast.res,file="../data/metacohort_assoc_contrasts_age_cohort_er_pr_her2_tsaeg_ln_anthraVSnoAnthra_GW.RData")
