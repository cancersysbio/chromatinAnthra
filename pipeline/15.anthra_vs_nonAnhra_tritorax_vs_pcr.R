# surv_antra vs non anthra tritorax
...
pheno_other = data.frame(rfs.t=pheno$os.t/365,rfs.e=pheno$os.e=="TRUE",anthra=pheno$anthra,age=pheno$age, tam=pheno$Tam,
                         er = as.factor(pheno$er),pr = as.factor(pheno$pr),her2 = as.factor(pheno$her2),
                         lymphNodePos= pheno$lympNodePos,t.stage=factor(pheno$t_stage),cohort=pheno$cohort,stringsAsFactors = F)


pheno.df=pheno_other
...
pheno.df$cohort = droplevels(pheno.df$cohort)
eset_1_COMBAT2 = mergedCOMBAT[mergedCOMBAT@featureData@data$ENTREZ_GENE_ID %in% na.omit(complex_trit$Entrez),]
...
for(i in 1:nrow(eset_1_COMBAT2)){
  ...
  #X.b$t.stage = droplevels(X.b$t.stage)
  dd <- datadist(X.b)
  options(datadist='dd')
  ...
  try({
    ...
    cxmodel2 = cph(Surv(rfs.t,rfs.e)~exp_c*anthra+rcs(age,3)+er+pr+her2+lymphNodePos+t.stage+MKI67_4,data = X.b,surv=T)
    
    anv = anova(cxmodel2)
    pval = anv[13,3]
    coef = cxmodel2$coefficients
    conf_int = confint(cxmodel2)
    coef_error = sqrt(diag(cxmodel2$var))
    ...
    mulvarrfs.res = rbind(mulvarrfs.res,c(probe = ezid,entrez = as.character(eset_1_COMBAT2@featureData@data$ENTREZ_GENE_ID[i]), 
                                          ...
                                          pval.expAnth=as.numeric(anv[13,3]),odds.expAnth=as.numeric(exp(coef)[13]),
                                          coef.expAntha = as.numeric(coef[13]),error.odds.expAnth = as.numeric(coef_error[13]),
                                          conf.l.expAntra = as.numeric(conf_int[13,1]),conf.h.expAntra = as.numeric(conf_int[13,2])
    ))
...
save( mulvarrfs.res,file="../data/metacohort_assoc_results_age_cohort_er_pr_her2_tsaeg_ln_anthraVSnoAnthra_V2_tritorax.RData")
save(contrast.res,file="../data/metacohort_assoc_contrasts_age_cohort_er_pr_her2_tsaeg_ln_anthraVSnoAnthra_V2_tritorax.RData")
